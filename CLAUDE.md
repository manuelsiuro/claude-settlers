...
- **Multiplayer (LAN)**: Deterministic lockstep architecture. `GameRng` (seeded mulberry32 PRNG) replaces all `Math.random()` in game logic. Fixed 50ms timestep via accumulator pattern in `Game.ts`. Command system (`Command.ts`, `CommandExecutor.ts`) routes all UI and AI state mutations through a single entry point (20 command types). `NetworkAdapter` interface with `LocalAdapter` (single-player) and `WebSocketAdapter` (multiplayer). Relay server in `server/` (Node.js + ws, ~300 lines). `LobbyPanel.ts` for room creation/joining with QR code sharing. Lockstep game loop: in multiplayer mode, simulation advances one fixed tick per turn packet from relay server. Checksum system hashes state every 10 turns for desync detection. `shared/` directory contains protocol types used by both client and server. Design doc: `docs/multiplayer.md`.
