...
- **Save migration chain**: `SaveLoad.ts` has `MIGRATIONS` record mapping version→migration function. `migrateSaveData()` runs all needed migrations sequentially. SAVE_VERSION=15.
...
- **Multiplayer (LAN)**: Deterministic lockstep architecture (2-4 players). `GameRng` (seeded mulberry32 PRNG) replaces all `Math.random()` in game logic. Fixed 50ms timestep via accumulator pattern in `Game.ts`. Command system (`Command.ts`, `CommandExecutor.ts`) routes all UI and AI state mutations through a single entry point (20 command types). `NetworkAdapter` interface with `LocalAdapter` (single-player), `WebSocketAdapter` (multiplayer), and `ReplayAdapter` (replay playback). Relay server in `server/` (Node.js + ws, auto-starts with `npm run dev`). `LobbyPanel.ts` for room creation/joining with QR code sharing, host/join tabs, player name persistence. Lockstep game loop: in multiplayer mode, simulation advances one fixed tick per turn packet from relay server. Checksum system every 10 turns with desync recovery (host sends full state snapshot). Reconnection via REJOIN_ROOM protocol with state sync. AI takeover for disconnected players. In-game chat (`ChatPanel.ts`). Multiplayer post-game stats (side-by-side player comparison). Replay system (save/load command logs, playback with speed control). `shared/` directory contains protocol types. Design doc: `docs/multiplayer.md`, user guide: `docs/multiplayer-guide.md`.
...
- **`server/`** — Multiplayer relay server: `index.ts` (HTTP + WebSocket entry point), `Room.ts` (room lifecycle, turn collection, checksum comparison). Node.js + ws. Runs no game logic — only relays commands between clients.
- **`shared/`** — Types shared between client and server: `protocol.ts` (ClientMessage, ServerMessage), `types.ts` (PlayerInfo, TurnPacket, RoomConfig).
...
- **`src/ui/ChatPanel.ts`** — In-game chat: 50-message log, Enter key toggle, player-colored names, unread badge.
- **`src/game/ReplayData.ts`** — Replay data format: config, seed, player assignments, commands by turn. Serialize/deserialize to JSON.
...
Phases 1–9 and all expansion phases (A–J) are complete, plus a comprehensive game review that added 35 features, and multiplayer Phases 1–3 (determinism foundation, LAN multiplayer, LAN robustness). The game now has 55 building types, 49 resource types, 45 unit types, hunger/morale systems (wired to production and combat), military expansion (5 unit types with rock-paper-scissors counters), advanced transport (multi-carry, road quality tiers), animal lifecycle, balance tuning, a full statistics dashboard with Canvas-based charts, standalone app packaging (Capacitor/Tauri/PWA), a comprehensive mobile UI overhaul, a barter marketplace with dynamic pricing and AI trading, a Living World feature with 6 ambient visual systems, spatial audio system, a polished map editor with map sharing (clipboard export/import), a 5-step interactive tutorial, building status diagnostics, 8 map scenarios, camera bookmarks, 13 random event types, adaptive AI with 4 personalities (Balanced/Economist/Militarist/Turtle), a diplomacy system (treaties: non-aggression/trade/alliance), 12 campaign scenarios with custom objectives, 24 cross-game achievements, sandbox mode, in-game encyclopedia, event log with camera navigation, loading screen with progress bar, auto-save (3 rotating slots), keyboard shortcuts (B/S/D/E/P/F1-F4/?), graphics presets (Low/Medium/High/Ultra), accessibility (3 colorblind modes + text scaling), animated water shader, night glow on buildings, smooth camera pan, fog of war edge softening, production chain status visualizer, production chain panel with swim-lane layout (14 chain groups), error boundaries for renderers, formal save migration chain (v3→v15), enhanced FPS counter with draw call stats, group attack commands, rally point support, LAN multiplayer with deterministic lockstep (relay server, lobby UI with QR sharing, command system with 20 command types, fixed timestep, seeded PRNG, desync recovery, reconnection with state sync, AI takeover on disconnect, in-game chat, 2-4 player support, multiplayer post-game stats, replay system), and a comprehensive UI polish pass (softened close buttons, encyclopedia card layout, semantic status color system with theme-aware CSS variables, ~30 inline hardcoded colors replaced, focus-visible outlines). See `docs/new-features.md` for the full guide, `docs/multiplayer.md` for the multiplayer architecture, and `PROGRESS.md` for history. 857 tests passing.
