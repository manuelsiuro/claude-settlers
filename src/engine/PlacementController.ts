...
export class PlacementController {
  ...
  selectBuilding(type: BuildingType): void {
    ...
    logger.debug('[Placement] selectBuilding:', type, 'isActive:', this.isActive);
  }
  ...
  private onTouchStart = (e: TouchEvent): void => {
    logger.debug('[Placement] touchstart', { selectedType: this.selectedType, touches: e.touches.length });
    if (!this.selectedType) return;
    const touch = e.touches[0];
    this.mouseDownPos = { x: touch.clientX, y: touch.clientY };
    this.mouseIsDown = true;
    this.updatePreview(touch.clientX, touch.clientY);
    logger.debug('[Placement] after updatePreview', { currentHex: this.currentHex, canPlace: this.canPlaceHere, ghost: !!this.ghostMesh });
  };
  ...
  private onTouchEnd = (e: TouchEvent): void => {
    logger.debug('[Placement] touchend', { mouseIsDown: this.mouseIsDown, selectedType: this.selectedType, currentHex: this.currentHex, canPlace: this.canPlaceHere });
    ...
    logger.debug('[Placement] touchend drag distance', dist);
    if (dist > CLICK_THRESHOLD) return;

    if (this.currentHex) {
      if (this.canPlaceHere) {
        logger.debug('[Placement] confirming placement!');
        this.confirmPlacement();
      } else if (this.currentPlacementError && this.selectedType) {
        logger.debug('[Placement] placement error:', this.currentPlacementError);
        this.onPlacementError?.(this.currentPlacementError, this.selectedType);
      }
    }
  };
