...
- [ ] 8.3 AI opponent — AIPlayer: runs each tick for non-human players. Heuristic priorities: (1) build economy (woodcutter→sawmill→food chain), (2) expand territory (place Guard Huts at border), (3) military (arm knights, attack weakest neighbor). Uses existing managers with AI playerId. Difficulty scales decision speed + build order quality.
- [ ] 8.4 Multi-player state — Remove hardcoded playerId=1. Game initializes N players with separate Castles (placed at map quadrants). Each player gets own UnitManager/ConstructionManager/LogisticsManager instances or pass playerId through existing managers. Territory colors per player. UI shows only human player's state.
- [ ] 8.5 Performance optimization — InstancedMesh for terrain tiles (1 draw call per terrain type instead of 1024). InstancedMesh for decorations (group by model type). Frustum culling for off-screen tiles. Benchmark at 64x64 map. Target: 60 FPS on mobile.
- [ ] 8.6 Save/load system — Serialize full game state to JSON: GameState (buildings, units), RoadNetwork (flags, roads), manager state (production progress, construction progress, transporter assignments), territory, knight data. Save to localStorage + download as file. Load restores all state and re-renders. Tests for round-trip serialization.
- [ ] 8.7 Sound effects & music — Web Audio API via Tone.js or raw AudioContext. Procedural/generated SFX: building placed, construction complete, combat clash, resource pickup, notification chime. Background ambient music loop. Volume controls in settings. Mute button.
- [ ] 8.8 Game speed & pause — Pause/resume button (spacebar shortcut). Game speed controls: 1x / 2x / 3x (scale dt multiplier). Speed indicator in UI. Pause overlay.
- [ ] 8.9 Integration test — Full game scenario: 2 players (human + AI), AI builds economy, territory expands, combat occurs, victory condition triggers. Save/load round-trip test. Performance benchmark test.
